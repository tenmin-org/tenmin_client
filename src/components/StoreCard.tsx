...
export const StoreCard = React.memo(function StoreCard({
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-center gap-3 p-4 bg-white dark:bg-zinc-800 rounded-2xl shadow-sm active:scale-[0.98] transition-transform cursor-pointer"
    >
      {store.image_url ? (
        <img
          src={store.image_url}
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm text-gray-500 dark:text-zinc-400 truncate">{store.address}</p>
      </div>

      <ChevronRight className="ml-auto text-gray-400 dark:text-zinc-500 flex-shrink-0" size={20} />
    </div>
  );
});
